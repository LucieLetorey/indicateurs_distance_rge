...
#Récupération géométrie zones résultant de la division de l'EPCI par la grille
...
petites_zones_plus=petites_zones%>%
  ...
  sf::st_as_sf()
#buffer un peu plus grand pour ne pas oublier tronçons
...
data_temp=bdd_dpe_logement_rel_batiment_groupe %>%
  ...
  dplyr::mutate(longitude_copy = longitude, latitude_copy = latitude)
...
#On crée un data frame avec les géométries de chaque celulle, puis on le transforme en objet SF. On attribue à chq cellule un ID.
...
res=sf::st_intersects(petites_zones_plus,departements_geo_simpl_lnglat)#Obtenir la liste des dpts touchés par la zone epci divisée +Buffer
...
table_na=rbind(table_na,data.frame(code = i,
                                   nb_na_bat=sum(data_temp2[indices_vide,]$nb),
                                   nb_bat_init=nb_l_bat,
                                   taux_perte_bat=(sum(data_temp2[indices_vide,]$nb)/nb_l_bat)*100,
...
save(resultat_etape2,file =paste0("//donnees.dreal-norm.ad.e2.rie.gouv.fr/services/SMCAP/05_BOeS/5PREL/base_donnees/BDNB_OPEN_SOURCE/bdnb/data/res_",i,".RData"))
}
...
par(mfrow=c(1,1))
plot(epci_geo_simpl_lnglat[271,]$geometry)
plot(grille,add=TRUE, border = "blue", lwd = 2)
plot(petites_zones[8,],add=TRUE,border="red")
plot(data_temp[452571,19],add=TRUE)
